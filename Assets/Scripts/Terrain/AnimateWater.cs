...
public class AnimateWater : MonoBehaviour {
    ...
    private Vector2 lwVector, lwNVector;
    ...
    void Update() {
      if (waterSyncWind == false) {
        ...
        if (windZone != null) {
          windZone.rotation = Quaternion.LookRotation(new Vector3(lwNVector.x, 0, lwNVector.y), Vector3.zero) * Quaternion.Euler(0, -90, 0);
        }
